...
class LexiconApiIntegrationTest {
    ...
    @Test
    fun `listCorpora hits the right path and parses snake_case fields`() = runTest {
        server.enqueue(
    ...
    @Test
    fun `askQuestion posts the question body to the corpus-scoped path`() = runTest {
        server.enqueue(
            MockResponse().setResponseCode(200).setBody(
                """
                {
    ...
    @Test
    fun `getCorpus URL-encodes the corpus id path segment`() = runTest {
        server.enqueue(
            MockResponse().setResponseCode(200).setBody(
                """
    ...
    @Test
    fun `configured static auth header is attached to every request`() = runTest {
    ...
    @Test
    fun `no auth header is sent when none is configured`() = runTest {
        server.enqueue(MockResponse().setResponseCode(200).setBody("[]"))
    ...
    @Test
    fun `a 4xx response surfaces as an HttpException rather than being swallowed`() = runTest {
        server.enqueue(
